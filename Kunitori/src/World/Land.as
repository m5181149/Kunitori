package World 
{
	...
	public class Land 
	{
		...
		//陸地か海かのフラグ
		const FRAG_SEA:int = 0;			//海
		const FRAG_LAND:int = 1;		//陸
		const FRAG_FOREST:int = 2;		//森
		const FRAG_MOUNTAIN:int = 3;	//山
		const FRAG_RIVER:int = 4;		//湖, 川
		const FRAG_COUNTRY:int = 5;		//国
...
//			var arraysize:int;
		...
		//大陸の中身を return
		public function returnArrayXY(x:int, y:int):int
		{
			return this.landdata[y][x];
		}
		
		//大陸の配列の大きさを return
		public function returnArraysize():int
		{
